...
dataframe1=data.frame(CUM=cumsum(dict$V3),CHR=dict$V2,LEN=dict$V3,stringsAsFactors = F)
dataframe2=data.frame(CUM=c(c(1:(sums/length_intervals))*length_intervals,sums),LEN=NA,CHR=NA,stringsAsFactors = F)
...
for(rowindex in c(1:nrow(dataframe))){
  if(grepl(pattern = "chr",x = dataframe[rowindex,"CHR"])){
    minus=dataframe[rowindex,"CUM"]
    dataframe[rowindex,"CUTOFF"]=dataframe[rowindex,"LEN"]
  }else{
    dataframe[rowindex,"CUTOFF"]=dataframe[rowindex,"CUM"]-minus
  }
}
##倒序标染色体
label=NA
